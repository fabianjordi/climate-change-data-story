...
def get_wind_data(station_name, time_back):
    ...
    query = """SELECT 
                ...
                windchill
                FROM "meteorology"."autogen"."{}" WHERE time <= now() - 8760h AND time >= now() - 8760h - {}""".format(station_name, time_back)
...
def get_daily_value_of_last_two_weeks(station_name, column_name):
    ...
    query = """SELECT 
                mean({})
                FROM "meteorology"."autogen"."{}"
                WHERE time <= now() - 8760h AND time >= now() - 8760h - 336h
                GROUP BY time(24h)""".format(column_name, station_name)
...
def get_3_hours_mean_value_of_last_five_days(station_name, column_name):
    ...
    query = """SELECT 
                mean({})
                FROM "meteorology"."autogen"."{}"
                WHERE time <= now() - 8760h AND time >= now() - 8760h - 120h
...
def get_single_column(station_name, column_name, back_from_now):
    ...
    query = """SELECT 
                {}
                FROM "meteorology"."autogen"."{}"
                WHERE time <= now() - 8760h AND time >= now() - 8760h - {}""".format(column_name, station_name, back_from_now)
...
def get_data_for_wind_warnings(station_name):
    ...
    query = """SELECT
                (wind_gust_max_10min)
                FROM "meteorology"."autogen"."{}"
                WHERE time <= now() - 8760h AND time >= now() - 8760h - 21d""".format(station_name)
...
def get_barometric_pressure_in_current_time_period(station_name):
    ...
    try:
        query = """SELECT 
                    ...
                    FROM "meteorology"."autogen"."{}"
                    WHERE time <= now() - 8760h AND time >= now() - 8760h - 4h GROUP BY time(1h) fill(previous)""".format(station_name)
        ...
        query = """SELECT 
                    mean(barometric_pressure_qfe) AS mean_barometric_pressure_qfe
                    FROM "meteorology"."autogen"."{}"
                    WHERE time < now() GROUP BY time(5d) fill(previous)""".format(station_name)
        ...
        while len_of_df <= 0:
            ...
            query = """SELECT
                    ...
                    WHERE
                        ...
                        AND
                        ()
                        (time <= now() - 8760h AND time >= now() - 8760h - 1d)""".format(
                station_name,
    ...
    while len_of_df <= 20:
        ...
        query = """SELECT
                ...
                WHERE
                    ...
                    (barometric_pressure_qfe > {} AND barometric_pressure_qfe < {})
                    AND
                    (time <= now() - 8760h AND time >= now() - 8760h - 3d)""".format(
            station_name,
